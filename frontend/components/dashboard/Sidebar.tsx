      ...
      <div className={`border-t border-white/5 shrink-0 ${isExpanded ? 'p-4' : 'py-4 flex flex-col items-center'}`}>
        <Link 
          ...
          className={`flex items-center rounded-lg text-sm font-medium transition-colors ${
            ...
            pathname === '/dashboard/settings' ? "bg-white/10 text-white" : "text-[#888] hover:bg-white/5 hover:text-white"
          }`}
        >
          <svg className={`shrink-0 ${isExpanded ? 'w-5 h-5' : 'w-[22px] h-[22px]'} ${pathname === '/dashboard/settings' ? 'text-white' : 'text-[#888]'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
