    ...
    <div className="bg-[#111217] rounded-2xl p-5 shadow-lg border border-white/5 h-full flex flex-col justify-between relative overflow-hidden group">
      ...
      <div className="flex justify-between items-start z-10 relative mb-4">
        <div className="flex gap-6">
          <div>
            <div className={`text-[10px] font-bold uppercase tracking-wider mb-0.5 transition-colors duration-500 ${activeTab === "recovered" ? "text-[#C8FF00]" : "text-zinc-400"}`}>Recovered</div>
            <div className="text-2xl font-bold text-white tracking-tight tabular-nums leading-none">
              <AnimatedNumber key={`rec-${animKeyRec}`} prefix="₹" value={rec.value} decimals={rec.decimals} suffix={rec.suffix} />
            </div>
          </div>
          <div>
            <div className={`text-[10px] font-bold uppercase tracking-wider mb-0.5 transition-colors duration-500 ${activeTab === "risk" ? "text-white" : "text-zinc-400"}`}>At Risk</div>
            <div className="text-2xl font-bold text-white tracking-tight tabular-nums leading-none">
      ...
      <div className="flex-1 relative w-full mt-2 z-0 min-h-[140px]">
        ...
        <div className="absolute bottom-[-15px] left-0 right-0 flex justify-between text-[10px] font-medium text-zinc-400 pt-2">
