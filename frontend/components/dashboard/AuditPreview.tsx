...
export default function AuditPreview({ data }: { data: ActionData }) {
  ...
  return (
    <div className="bg-[#111217] rounded-2xl border border-white/5 h-full p-5 flex flex-col justify-center relative overflow-hidden">
      <div className="absolute top-0 right-0 w-24 h-24 bg-[#FF9500]/5 rounded-full blur-[30px] pointer-events-none"></div>
      <h3 className="text-[#888] font-bold text-[10px] tracking-widest uppercase mb-4">Action Health</h3>
      <div className="space-y-4 relative z-10">
        <div className="flex justify-between items-end">
          <div>
            <div className="text-[13px] font-black text-white">Success Rate</div>
            <div className="text-[8px] text-[#666] uppercase tracking-widest">{data.successful} of {data.totalActions}</div>
          </div>
        ...
        <div className="flex justify-between items-end mt-4">
          <div>
            <div className="text-[13px] font-black text-white">Failed</div>
            <div className="text-[8px] text-[#666] uppercase tracking-widest">{data.failed} actions</div>
          </div>
