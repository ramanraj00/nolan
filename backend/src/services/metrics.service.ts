...
export class MetricsService {
  /**
   * Calculates the primary recovery metrics for a given merchant.
   * - totalRevenueAtRisk: Sum of revenue_at_risk for all recovery cases
   * - recoveredRevenue: Sum of revenue_at_risk for all RECOVERED recovery cases
   * - recoveryRate: (recoveredRevenue / totalRevenueAtRisk) * 100
   * - failedPayments: Count of all FAILED payments
   */
  static async getMerchantRecoveryMetrics(merchantId: string) {
    // 1. Verify if merchant actually exists
    ...
    // 3. Query for 7-day trend
    const trendQuery = `
      SELECT 
        TO_CHAR(date_series.date, 'YYYY-MM-DD') AS date,
        COALESCE(SUM(rc.revenue_at_risk), 0) AS "recoveredRevenue"
      FROM generate_series(CURRENT_DATE - INTERVAL '6 days', CURRENT_DATE, '1 day'::interval) AS date_series(date)
    ...
    // 4. Query for recovery cases status breakdown
    const statusQuery = `
    ...
    // Initialize all valid statuses to 0
    ...
    // 5. Query for failure reason breakdown
    const reasonQuery = `
    ...
    // 7. Query for Policy Performance
    const policyQuery = `
      SELECT 
    ...
    // 8. Query for Action Performance
    const actionQuery = `
      SELECT 
    ...
    // Sometimes PG AVG returns strings, so cast to number.
    // Also, probability is stored as 0.00 to 1.00, so multiply by 100 for percentage output.
